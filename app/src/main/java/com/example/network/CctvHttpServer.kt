    ...
    companion object {
        fun getLocalIpAddress(): String {
            try {
                ...
                val allIps = mutableListOf<Pair<String, String>>() // name to ip
                while (interfaces.hasMoreElements()) {
                    ...
                    while (addresses.hasMoreElements()) {
                        ...
                        if (!addr.isLoopbackAddress && addr is Inet4Address) {
                            ...
                            allIps.add(Pair(intf.name.lowercase(), ip))
                        }
                    }
                }

                // 1. Prefer Wi-Fi / Hotspot interfaces (wlan0, ap0, softap, rndis)
                val wifiOrApIp = allIps.firstOrNull { (name, ip) ->
                    (name.contains("wlan") || name.contains("ap") || name.contains("swlan") || name.contains("softap")) &&
                            (ip.startsWith("192.168.") || ip.startsWith("10.") || ip.startsWith("172."))
                }?.second

                if (wifiOrApIp != null) return wifiOrApIp

                // 2. Prefer any private RFC1918 address (192.168.x.x, 10.x.x.x, 172.16-31.x.x)
                val privateIp = allIps.firstOrNull { (_, ip) ->
                    ip.startsWith("192.168.") || ip.startsWith("10.") || ip.startsWith("172.")
                }?.second
                ...
                val anyIp = allIps.firstOrNull()?.second
